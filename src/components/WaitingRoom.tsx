...
export default function WaitingRoom({ interviewId, onStartInterview }: WaitingRoomProps) {
  ...
  const handleStart = async () => {
    ...
    try {
      const response = await fetch(`/api/interviews/${interview.id}/start`, { method: 'POST' });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || 'Failed to start the interview.');
      ...
      setPermissionError(err.message || 'Failed to initialize interview session.');
    }
  };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-neutral-bg p-4 font-sans">
        <div className="w-full max-w-md bg-white p-8 rounded-xl shadow-lg border border-neutral-200 text-center">
          <AlertTriangle className="h-12 w-12 text-muted-accent mx-auto mb-4" />
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5] py-12 px-4 sm:px-6 lg:px-8 font-sans" id="waiting-room-view">
      <div className="mx-auto max-w-4xl grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <div className="md:col-span-5 space-y-6">
          <div className="bg-white p-6 rounded-xl border border-neutral-200 shadow-sm space-y-4">
            <div>
              <span className="font-mono text-[9px] font-bold uppercase tracking-widest text-accent bg-accent/10 border border-accent/20 px-2.5 py-1 rounded">
            ...
            <div className="border-t border-neutral-100 pt-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 pt-1">
                <div>
                  ...
                  <span className="text-xs font-bold text-accent uppercase tracking-wider font-mono">{interview?.interviewType}</span>
                </div>
                <div>
          ...
          <div className="bg-accent/5 rounded-lg border border-accent/15 p-4 text-xs text-ink/70 leading-relaxed space-y-1.5 shadow-sm">
            <p className="font-bold text-accent font-sans">Session Guideline:</p>
        ...
        <div className="md:col-span-7 space-y-6">
          <div className="bg-white p-6 rounded-xl border border-neutral-200 shadow-sm space-y-4">
            ...
            <div className="aspect-video w-full bg-slate rounded-xl overflow-hidden relative border border-graphite shadow-md">
              {permissionsGranted && mediaStream ? (
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center text-neutral-bg space-y-3">
                  ...
                  <p className="text-xs text-white/70 max-w-xs font-sans">Webcam live feed will activate instantly once browser equipment permissions are granted.</p>
                  <button
                    onClick={requestPermissions}
                    className="flex items-center gap-2 rounded-lg bg-accent hover:opacity-90 text-white px-4 py-2.5 text-xs font-bold transition-all duration-200 cursor-pointer shadow-md shadow-accent/15"
                  >
            ...
            {/* Error notifications */}
            {permissionError && (
              <div className="flex items-start gap-3 rounded-lg bg-muted-accent/5 p-4 text-xs text-muted-accent border border-muted-accent/20 animate-shake">
                <ShieldAlert className="h-5 w-5 shrink-0 text-muted-accent mt-0.5" />
            ...
            <div className="grid grid-cols-2 gap-4 text-xs pt-2">
              <div className={`flex items-center gap-2 p-3 rounded-lg border transition-all duration-300 ${permissionsGranted ? 'bg-accent/10 border-accent/20 text-accent' : 'bg-neutral-bg border-neutral-200 text-ink/30'}`}>
            ...
            <button
              ...
              className="w-full rounded-lg bg-accent px-4 py-3 text-sm font-bold text-white hover:opacity-95 focus:outline-none focus:ring-2 focus:ring-accent/50 disabled:bg-neutral-bg disabled:text-ink/30 disabled:border-neutral-200 disabled:cursor-not-allowed transition-all duration-200 cursor-pointer flex justify-center items-center gap-2 shadow-md shadow-accent/15"
            >
