...
export default function CreateInterview({ onInterviewCreated }: CreateInterviewProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-neutral-200 p-6 shadow-sm" id="create-interview-view">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-neutral-200 pl-10 pr-4 py-2.5 text-sm text-ink placeholder-ink/30 focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent transition-colors bg-neutral-bg/20"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              className="w-full rounded-lg border border-neutral-200 px-4 py-2.5 text-sm text-ink focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent transition-colors bg-white cursor-pointer"
            >
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-neutral-200 pl-10 pr-4 py-2.5 text-sm text-ink focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent transition-colors bg-neutral-bg/20"
              />
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div 
            className={`border-2 border-dashed rounded-lg p-5 text-center cursor-pointer transition-all duration-200 ${
              cvFile 
                ...
                : 'border-neutral-200 hover:border-accent hover:bg-neutral-bg/30'
            }`}
            onDragOver={(e) => {
              e.preventDefault();
            }}
          ...
          >
            ...
            />
            {cvFile ? (
              <div className="flex items-center justify-center gap-2.5 text-accent font-semibold text-sm">
              ...
              <div className="text-ink/60 text-xs">
                <p className="font-semibold">Drag & drop candidate CV/Resume here, or <span className="text-accent hover:underline">browse</span></p>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full rounded-lg bg-accent px-4 py-2.5 text-sm font-semibold text-white hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-accent/50 disabled:opacity-50 transition-all duration-200 cursor-pointer flex justify-center items-center shadow-md shadow-accent/15"
        >
          {loading ? (
      ...
      {/* Sharing Link Box */}
      {createdLink && (
        <div className="mt-6 p-4 rounded-lg bg-accent/5 border border-accent/20 flex flex-col md:flex-row md:items-center justify-between gap-3 animate-fade-in">
          <div className="min-w-0 flex-1">
            <span className="text-[10px] font-bold text-accent uppercase tracking-wider block mb-1 font-mono">
          ...
          >
            {copied ? (
              <>
                <Check className="h-4 w-4 text-white" /> Copied!
              </>
            ) : (
              <>
