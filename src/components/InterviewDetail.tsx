...
export default function InterviewDetail({ interviewId, onDeleteSuccess }: InterviewDetailProps) {
  ...
  const getScoreColor = (score: number) => {
    if (score >= 8) return 'text-accent bg-accent/10 border-accent/25';
    if (score >= 6) return 'text-muted-accent bg-muted-accent/10 border-muted-accent/25';
    return 'text-neutral-500 bg-neutral-500/10 border-neutral-500/25';
  };

  const getScoreBarBg = (score: number) => {
    if (score >= 8) return 'bg-accent';
    if (score >= 6) return 'bg-muted-accent';
  ...
  return (
    <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-6 sm:p-8 overflow-hidden" id="interview-dossier">
      ...
      <div className="flex flex-col sm:flex-row items-start justify-between gap-4 mb-8 border-b border-neutral-100 pb-6">
        <div>
          <span className="font-mono text-[10px] font-bold uppercase tracking-wider text-accent bg-accent/10 border border-accent/20 px-2.5 py-1 rounded">
          ...
          {interview.cvFileUrl && (
            <div className="mt-3">
              <a
                ...
                className="inline-flex items-center gap-1.5 text-xs font-bold text-accent bg-accent/5 hover:bg-accent/10 px-3 py-1.5 rounded-lg transition-colors border border-accent/20 font-mono"
              >
        ...
        <div className="text-left sm:text-right shrink-0">
          {!isCompleted ? (
            <div className="flex flex-col items-start sm:items-end gap-2">
              <div className="inline-flex items-center px-3 py-1 bg-muted-accent/10 text-muted-accent rounded-full font-bold text-[10px] uppercase tracking-wider border border-muted-accent/20 font-mono">
                <span className="h-1.5 w-1.5 rounded-full bg-muted-accent animate-pulse mr-1.5 inline-block" />
              ...
              >
                <Trash2 className="h-3.5 w-3.5 text-muted-accent" /> Delete Interview
              </button>
            </div>
          ) : (
            <div className="flex flex-col items-start sm:items-end gap-2">
              {interview.assessmentStatus === 'failed' ? (
                ...
                <div className="inline-flex items-center px-3 py-1 bg-accent text-white rounded-full font-bold text-[10px] uppercase tracking-wider border border-accent shadow-sm font-mono">
                  Strong Hire
                </div>
              ) : (
                <div className="inline-flex items-center px-3 py-1 bg-neutral-500 text-white rounded-full font-bold text-[10px] uppercase tracking-wider border border-neutral-600 shadow-sm font-mono animate-pulse">
                  No-Hire / Reject
                </div>
              )}
              <button
          ...
          {isCompleted && interview.assessmentStatus !== 'failed' && (
            <div className={`text-3xl sm:text-4xl font-black mt-2 font-mono ${interview.decision === 'hire' ? 'text-accent' : 'text-neutral-500'}`}>
              {averageScore}
        ...
        {activeTab === 'report' ? (
          ...
          <div className="space-y-6">
            {!isCompleted ? (
              ...
              <div className="text-center py-16 bg-muted-accent/5 rounded-xl border border-muted-accent/20 p-6 max-w-2xl mx-auto">
                <AlertTriangle className="h-10 w-10 text-muted-accent mx-auto mb-3" />
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl border border-neutral-200 p-6 space-y-3">
                  <h3 className="text-[10px] font-bold uppercase tracking-widest text-ink/40 flex items-center gap-2 font-mono">
                    <CheckCircle className={`h-4 w-4 ${interview.decision === 'hire' ? 'text-accent' : 'text-neutral-500'}`} /> Decision Justification
                  </h3>
                ...
                <div className="bg-white rounded-xl border border-neutral-200 p-6 space-y-5">
                  <h3 className="text-[10px] font-bold uppercase tracking-widest text-ink/40 flex items-center gap-2 font-mono">
                    <Award className="h-4 w-4 text-accent" /> Skill Assessment
                  </h3>
                  <div className="space-y-6">
          ...
          <div className="space-y-6">
            ...
            <div>
              <h3 className="text-[10px] font-bold uppercase tracking-widest text-ink/40 mb-3 flex items-center gap-2 font-mono">
                <Video className="h-4 w-4 text-accent" /> Candidate Session Recording
              </h3>
              {interview.recordingUrl ? (
                ...
                <div className="p-12 rounded-xl bg-muted-accent/5 border border-muted-accent/20 text-center flex flex-col items-center justify-center animate-pulse">
                  <svg className="animate-spin h-8 w-8 text-muted-accent mb-3" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                  </svg>
                  <p className="text-sm font-bold text-muted-accent">Uploading recording...</p>
                ...
                <div className="p-12 rounded-xl bg-accent/5 border border-accent/20 text-center flex flex-col items-center justify-center animate-pulse">
                  <svg className="animate-spin h-8 w-8 text-accent mb-3" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                  </svg>
                  <p className="text-sm font-bold text-accent">Processing Evaluation...</p>
                </div>
              ) : (
            ...
            <div>
              <h3 className="text-[10px] font-bold uppercase tracking-widest text-ink/40 mb-4 flex items-center gap-2 font-mono">
                <MessageSquare className="h-4 w-4 text-accent" /> Dialogue Timeline Transcript
              </h3>
